...
+ function($) {
  ...
  var MinuteNumber = function(DefaultValue) {
    var arr = [];
    for (var i = 1; i <= 59; i++) {
      if (i % DefaultValue == 0) {
  ...
  var initYears = function (custoYearsStart,custoYearsEnd) {
    var arr = [];
    for (var i = custoYearsStart; i <= custoYearsEnd; i++) { arr.push(i); }
    return arr;
  };
  ...
  var defaults = {
    ...
    cols: [
      ...
      // Minutes
      {
        values: MinuteNumber(3),
      }
    ],
    custoMminutes: 1,
    custoYearsStart:1950,
    custoYearsEnd:2050
  };
  ...
  $.fn.datetimePicker = function(params) {
    return this.each(function() {
      ...
      p.cols[0].values=initYears(p.custoYearsStart,p.custoYearsEnd);
      p.cols[6].values = MinuteNumber(p.custoMminutes);
      $(this).picker(p);
